...
void	imprimir_tabla(int n, double nodose[n + 1], double nodosc[n + 1], double mate[n + 1][n + 1], double matc[n + 1][n + 1])
{
	...
	double mayoreq = 0;             //guardamos aqui el error mas grande con equidistantes
	double mayorcheby = 0;          //guardamos aqui el error mas grande con chebyshev
	...
	{
		x += 0.011;
		if (fabs(f(x) - P(n, x, nodose, mate)) > mayoreq)
            mayoreq = fabs(f(x) - P(n, x, nodose, mate));
        if (fabs(f(x) - P(n, x, nodosc, matc)) > mayorcheby)
            mayorcheby = fabs(f(x) - P(n, x, nodosc, matc));
	}
	printf("   %.16G	             %.16G\n", mayoreq, mayorcheby);
}
